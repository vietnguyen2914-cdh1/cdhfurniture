	...
	class StaticBlockContent {
		...
		static function init_static_blocks() {
			if (function_exists('register_post_type')) {
				register_post_type('static_block',
					array(
						'labels' => array(
							'name' 					=>	esc_html_x('Static Content Blocks', 'post type general name', 'unicase-extensions'),
							'singular_name' 		=>	esc_html_x('Static Block', 'post type singular name', 'unicase-extensions'),
							'add_new' 				=>	esc_html_x('Add New', 'block', 'unicase-extensions'),
							'add_new_item' 			=>	esc_html__('Add New Block', 'unicase-extensions'),
							'edit_item' 			=>	esc_html__('Edit Block', 'unicase-extensions'),
							'new_item' 				=>	esc_html__('New Block', 'unicase-extensions'),
							'all_items' 			=>	esc_html__('Static Blocks', 'unicase-extensions'),
							'view_item' 			=>	esc_html__('View Block', 'unicase-extensions'),
							'search_items' 			=>	esc_html__('Search', 'unicase-extensions'),
							'not_found' 			=>	esc_html__('No blocks found', 'unicase-extensions'),
							'not_found_in_trash' 	=>	esc_html__('No blocks found in Trash', 'unicase-extensions'), 
							'parent_item_colon' 	=> '',
		...
		static function get_static_content($args=array()) {
			...
			if (is_null($page_data))
				...
				$content = apply_filters( 'unicase_static_content', $content );
...
global $unicase_meta_box_static_blocks;

// Define Meta Fields
//................................................................
$unicase_meta_box_static_blocks = array(
	'id' 		=> 'theme-meta-box-static-block-filters',
	'title' 	=>  esc_html__('Content Options', 'unicase-extensions'),
	'page' 		=> 'static_block',
	'context' 	=> 'side',
	'priority' 	=> 'default',
	'fields' 	=> array(
    	array(
    	   'name' 		=> esc_html__('Content Filters', 'unicase-extensions'),
    	   'desc' 		=> esc_html__('Apply all WP content filters? This will include plugin added filters.', 'unicase-extensions'),
    	   'id' 		=> 'content_filters',
    	   'type' 		=> 'radio',
    	   'std' 		=> '',
    	   'options'	=> array(
    	   		'default'	=> esc_html__('Defaults (recommended)', 'unicase-extensions'),
    	   		'all' 		=> esc_html__('All Content Filters', 'unicase-extensions')
    	   	)
    	),
    	array(
    	   'name' 		=> esc_html__('Auto Paragraphs', 'unicase-extensions'),
    	   'desc' 		=> esc_html__('Add &lt;p&gt; and &lt;br&gt; tags automatically.(disabling may fix layout issues)', 'unicase-extensions'),
    	   'id' 		=> 'wpautop',
    	   'type' 		=> 'radio',
    	   'std' 		=> '',
    	   'options' 	=> array(
    	   		'on' 	=> esc_html__('On', 'unicase-extensions'),
    	   		'off' 	=> esc_html__('Off', 'unicase-extensions')
    	   	)
    	)
   	)
);
...
function unicase_add_box_static_blocks() {
	global $unicase_meta_box_static_blocks;
	
	add_meta_box(
		$unicase_meta_box_static_blocks['id'], 
		$unicase_meta_box_static_blocks['title'], 
		'unicase_show_box_static_blocks', 
		$unicase_meta_box_static_blocks['page'], 
		$unicase_meta_box_static_blocks['context'], 
		$unicase_meta_box_static_blocks['priority']
	);

}

add_action('admin_menu', 'unicase_add_box_static_blocks');
...
function unicase_show_box_static_blocks() {
	global $unicase_meta_box_static_blocks, $post;
 	
	// Use nonce for verification
	echo '<input type="hidden" name="unicase_meta_box_nonce" value="', wp_create_nonce( plugin_basename(__FILE__) ), '" />';
  
  	$increment = 0;
	foreach ($unicase_meta_box_static_blocks['fields'] as $field) {
		// some styling
...
add_action('save_post', 'unicase_save_data_static_blocks');
...
function unicase_save_data_static_blocks($post_id) {
	global $unicase_meta_box_static_blocks;
 
	// verify nonce
	if ( !isset($_POST['unicase_meta_box_nonce']) || !wp_verify_nonce($_POST['unicase_meta_box_nonce'], plugin_basename(__FILE__))) {
		return $post_id;
	}
